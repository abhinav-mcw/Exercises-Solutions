...
std::string kernelsource =
    ...
    "{                                                                      \n"
      ...
      " \n"
      " \n"
    "__kernel void optimized_rowCPerWorkItem_mmul(                                                    \n"
    "   const int N,                                                        \n"
    "   __global float* A,                                                  \n"
    "   __global float* B,                                                  \n"
    "   __global float* C)                                                  \n"
    ...
    "\n"
    "\n"
        "__kernel void optimized_rowCPerWorkItemAPrivate_mmul(                                                    \n"
    "   const int N,                                                        \n"
    "   __global float* A,                                                  \n"
    "   __global float* B,                                                  \n"
    "   __global float* C)                                                  \n"
...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    // //--------------------------------------------------------------------------------
    // // OpenCL matrix multiplication ... Optimised C row per work item
    // //--------------------------------------------------------------------------------

    // timer.reset();

    // // Create the compute kernel from the program
    // cl::make_kernel<int, cl::Buffer, cl::Buffer, cl::Buffer> optimized_rowCPerWorkItem_mmul(program,
    //                                                                  "optimized_rowCPerWorkItem_mmul");

    // printf(
    //     "\n===== OpenCL, optmised memory matrix mult, C(i) per work item, order %d ======\n",
    //     N);
    ...
    //   optimized_rowCPerWorkItem_mmul(cl::EnqueueArgs(queue, cl::NDRange(N), cl::NDRange(ORDER/16)), N, d_a, d_b, d_c);
    ...
    // --------------------------------------------------------------------------------
    // OpenCL matrix multiplication ... Optimised C row per work item, A private Memory
    // --------------------------------------------------------------------------------

    timer.reset();

    // Create the compute kernel from the program
    cl::make_kernel<int, cl::Buffer, cl::Buffer, cl::Buffer> optimized_rowCPerWorkItemAPrivate_mmul(program,
                                                                     "optimized_rowCPerWorkItemAPrivate_mmul");

    printf(
        "\n===== OpenCL, optmised memory matrix mult, C(i) per work item, A row private order %d ======\n",
        N);
    ...
    for (int i = 0; i < COUNT; i++)
    {
      ...
      optimized_rowCPerWorkItemAPrivate_mmul(cl::EnqueueArgs(queue, cl::NDRange(N), cl::NDRange(ORDER/16)), N, d_a, d_b, d_c);
